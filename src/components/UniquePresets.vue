<template>
  <div
    class="fixed left-1/2 -translate-x-1/2 transition duration-500 origin-bottom ease-out bottom-10 z-10 w-full max-w-full h-full max-h-[300px]  2xl:max-h-[600px] sm:max-w-screen-xl px-4 sm:px-6 md:px-8 font-inter"
    :class="[showUniquePreset ? 'scale-100 ' : 'scale-0']"
  >
    <div
      class="w-full h-full bg-white dark:bg-custom-dark-400 border dark:border-gray-200 rounded-lg"
    >
      <div
        ...
        >
          ...
          <div class="w-full grid grid-cols-2 bg-gradient lg:grid-cols-4 gap-4">
            <button
              ...
              @click="handlePreset(preset.color, preset.direction, preset.from, preset.via, preset.to)"
  ...
  >
    <svg
      ...
      stroke-linecap="round"
      stroke-linejoin="round"
      class="size-3 sm:size-4 animate-spin-slow bg-green-400"
    >
      <path
...
const handlePreset = (presetColor: string, direction: string, from: string, via: string, to: string) => {
